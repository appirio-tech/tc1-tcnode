...
<%-- Center Column Begins --%>
        <td width="100%" align="left" class="bodyColumn">
            ...
            <div>
            <strong>
            <a href="/tc?module=Static&amp;d1=introevent&amp;d2=bcsscs&amp;d3=revealTriangle" class="bcLink">Problem Statement</a>
            | Solution
            </strong>
            </div>
...
public class RevealTriangle {
  ...
  public String[] calcTriangle(String[] questionMarkTriangle) {
    char[][] triangle = new char[questionMarkTriangle.length][];
    
    for (int y = 0;y&lt;triangle.length;y++) {
      triangle[y] = questionMarkTriangle[y].toCharArray();
    }
        ...
        for (int x = 0;x&lt;triangle[y].length;x++) {
          if (triangle[y][x]!='?') {
            if ((triangle[y-1][x]!='?')&amp;&amp;(triangle[y-1][x+1]=='?')) {
              triangle[y-1][x+1] = (char) ((((triangle[y][x]-'0')+10-(triangle[y-1][x]-'0'))%10)+'0');
              continue outer;
            } else if ((triangle[y-1][x]=='?')&amp;&amp;(triangle[y-1][x+1]!='?')) {
              triangle[y-1][x] = (char) ((((triangle[y][x]-'0')+10-(triangle[y-1][x+1]-'0'))%10)+'0'); 
              continue outer;
            }
          }
        }
    ...
    String[] result = new String[questionMarkTriangle.length];
    
    for (int y = 0;y&lt;triangle.length;y++) {
      result[y] = new String(triangle[y]);
    }
