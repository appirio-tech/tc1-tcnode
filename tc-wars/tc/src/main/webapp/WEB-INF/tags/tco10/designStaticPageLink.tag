...
<c:set var="url"
       value="/tc?${requestScope.MODULE_KEY}=Static&amp;d1=tournaments&amp;d2=tco10&amp;d3=design&amp;d4=${page}"/>

<c:choose>
    <c:when test="${page eq 'leaderboard'}">
        <c:set var="url" value="/tco10?${requestScope.MODULE_KEY}=ContestDetails&amp;eid=3432&amp;ct=476"/>
    </c:when>
</c:choose>
