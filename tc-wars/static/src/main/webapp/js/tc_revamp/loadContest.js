			...
			if (data.jsonErrors.length == 0) {
				...
				if (category == "ActiveContests") {
					...
					$.each(data.contests, function (i, item) {
						tbody.append("<tr></td><td class='first leftAligned'><a href='" + getContestLink(item) + "'>" + item.contestName + "</td><td class='leftAligned'>" + item.type + "</td><td>" + item.catalog + "</td><td>Not Set</td><td class='leftAligned'>" + item.status + "</td><td class='leftAligned'><a href='" + getRegDetailLink(item) + "'>" + item.numberOfRegistrants + "</a></td><td class='last'><a href='" + getContestLink(item) + "'>Details</a></td></tr>");
					});
