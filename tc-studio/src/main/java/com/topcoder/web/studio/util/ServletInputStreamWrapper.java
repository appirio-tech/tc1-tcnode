...
/**
 * <p>A wrapper around the input stream. The main goal of such a wrapper is to notify </p>
 * 
 * @author isv
...
public class ServletInputStreamWrapper extends ServletInputStream {
    ...
    /**
     * <p>A <code>ServletInputStream</code> wrapped by this wrapper.</p>
     */
    private ServletInputStream stream;

    /**
     * <p>A <code>SubmissionUploadListener</code> to be notified on submission uploading progress. Optional.</p>
     */
    private SubmissionUploadListener listener;

    /**
     * <p>Constructs new <code>ServletInputStreamWrapper</code> instance to be used for notifying the specified listener
     * on number of bytes read from the specified stream.</p>
     * 
     * @param stream a <code>ServletInputStream</code> to be wrapped by this wrapper.
     * @param listener a <code>SubmissionUploadListener</code> to be notified on number of bytes read from the specified
     *        stream.
     */
    public ServletInputStreamWrapper(ServletInputStream stream, SubmissionUploadListener listener) {
        this.stream = stream;
        this.listener = listener;
    }

    /**
     * <p>Reads the line of data from stream into specified buffer.</p>
     * 
     * @param b a <code>byte</code> array to read data into. 
     * @param off an <code>int</code> specifying the offset.
     * @param len an <code>int</code> specifying the number of bytes to read.
    ...
    public int readLine(byte[] b, int off, int len) throws IOException {
        int bytesRead = this.stream.readLine(b, off, len);
        if ((this.listener != null) && (bytesRead != -1)) {
            this.listener.onRead(bytesRead);
        }
        return bytesRead;    
    }
    ...
    public int read() throws IOException {
        ...
        if ((this.listener != null) && (byteRead != -1)) {
            this.listener.onRead(1);
        }
        return byteRead;    
    }
     ...
     *
     * <p> If <code>b</code> is <code>null</code>, a <code>NullPointerException</code> is thrown.  If the length of
     * <code>b</code> is zero, then no bytes are read and <code>0</code> is returned; otherwise, there is an attempt to
     * read at least one byte. If no byte is available because the stream is at end of file, the value <code>-1</code>
     * is returned; otherwise, at least one byte is read and stored into <code>b</code>.
     *
     * <p> The first byte read is stored into element <code>b[0]</code>, the next one into <code>b[1]</code>, and so on.
     * The number of bytes read is, at most, equal to the length of <code>b</code>. Let <i>k</i> be the number of bytes
     ...
     * actually read; these bytes will be stored in elements <code>b[off]</code> through
     ...
     * thrown. In particular, an <code>IOException</code> is thrown if the input stream has been closed.
     *
     * <p> The <code>read(b,</code> <code>off,</code> <code>len)</code> method for class <code>InputStream</code> simply
     ...
     * of bytes read before the exception occurred is returned.  Subclasses are encouraged to provide a more efficient
     ...
     * are skipped.
     *
     * <p> The <code>skip</code> method of <code>InputStream</code> creates a byte array and then repeatedly reads into
     * it until <code>n</code> bytes have been read or the end of the stream has been reached. Subclasses are encouraged
     ...
     *
     * <p> The <code>available</code> method for class <code>InputStream</code> always returns <code>0</code>.
     *
     ...
     * of the <code>read</code> method depend on the particular type of the input stream. </ul></ul>
     *
     * <p>The method <code>reset</code> for class <code>InputStream</code> does nothing except throw an
