...
import com.topcoder.web.aolicq.model.ContestConfig;
...
public abstract class Base extends ShortHibernateProcessor {
    ...
    protected void loadGeneralEditContestData() throws Exception {
        getRequest().setAttribute("docTypes", AolicqDAOUtil.getFactory().getDocumentTypeDAO().getDocumentTypes());
        getRequest().setAttribute("contestStatuses", AolicqDAOUtil.getFactory().getContestStatusDAO().getContestStatuses());
        getRequest().setAttribute("fileTypes", AolicqDAOUtil.getFactory().getFileTypeDAO().getFileTypes());
        ...
        getRequest().setAttribute("prizeTypes", AolicqDAOUtil.getFactory().getPrizeTypeDAO().getPrizeTypes());
    ...
    protected ResultSetContainer getForumList() throws Exception {
        ...
        DataAccessInt da = new DataAccess(DBMS.AOLICQ_DATASOURCE_NAME);
    ...
    protected void loadEditContestData(Contest contest) throws Exception {
        ...
        ContestPropertyDAO dao = AolicqDAOUtil.getFactory().getContestPropertyDAO();
        ContestConfig temp;
        ...
        for (Object anA : a) {
            fileTypes.add(((AolicqFileType) anA).getId().toString());
        }
