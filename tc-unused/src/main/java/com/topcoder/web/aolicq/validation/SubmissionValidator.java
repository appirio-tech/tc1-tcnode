...
public class SubmissionValidator implements Validator {
    ...
    public static MimeType getMimeType(UploadedFile submission) throws FileDoesNotExistException, PersistenceException {
        MimeType mt = AolicqDAOUtil.getFactory().getMimeTypeDAO().find(submission.getContentType());
        if (mt == null) {
            ...
            for (AolicqFileType ft : AolicqDAOUtil.getFactory().getFileTypeDAO().getFileTypes()) {
