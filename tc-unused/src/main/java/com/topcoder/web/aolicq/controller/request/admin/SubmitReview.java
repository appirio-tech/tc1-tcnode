...
/**
 * @author dok
 * @version $Revision: 73818 $ Date: 2005/01/01 00:00:00
...
public class SubmitReview extends Base {
    protected void dbProcessing() throws Exception {
        ...
        ReviewStatus rs = AolicqDAOUtil.getFactory().getReviewStatusDAO().find(status);
        if (rs == null) {
            addError(Constants.REVIEW_STATUS_ID, "Invalid review status specified");
        }
        if (hasErrors()) {
            getRequest().setAttribute("submission", AolicqDAOUtil.getFactory().getSubmissionDAO().find(submissionId));
            SubmissionReview submissionReview = AolicqDAOUtil.getFactory().getSubmissionReviewDAO().find(submissionId);
            ...
            SubmissionReview sr = AolicqDAOUtil.getFactory().getSubmissionReviewDAO().find(submissionId);
            if (sr == null) {
            ...
            AolicqDAOUtil.getFactory().getSubmissionReviewDAO().saveOrUpdate(sr);
...
*/
            ...
            sr = AolicqDAOUtil.getFactory().getSubmissionReviewDAO().find(submissionId);
/*
