...
import com.topcoder.web.aolicq.validation.*;
...
public class EditContest extends Base {
    ...
    protected void dbProcessing() throws Exception {
        ...
        if ("".equals(StringUtils.checkNull(contestStatusId))) {
            ...
            status = AolicqDAOUtil.getFactory().getContestStatusDAO().find(new Integer(contestStatusId));
            if (status == null) {
                addError(Constants.CONTEST_STATUS_ID, "Please choose a valid contest status.");
            }
        }
        ...
        if (hasErrors()) {
            ...
            if (!"".equals(StringUtils.checkNull(contestId))) {
                setDefault(Constants.CONTEST_STATUS_ID,
                        AolicqDAOUtil.getFactory().getContestDAO().find(new Long(contestId)).getStatus().getId());
            } else if (status != null) {
            ...
            if (!"".equals(StringUtils.checkNull(contestId))) {
                ...
                contest = AolicqDAOUtil.getFactory().getContestDAO().find(new Long(contestId));
            } else {
            ...
            FileTypeDAO fDao = AolicqDAOUtil.getFactory().getFileTypeDAO();
            HashSet<AolicqFileType> fts = new HashSet<AolicqFileType>();
            ...
            if (log.isDebugEnabled()) {
                closeConversation();
                beginCommunication();
                Contest myContest = AolicqDAOUtil.getFactory().getContestDAO().find(contest.getId());
