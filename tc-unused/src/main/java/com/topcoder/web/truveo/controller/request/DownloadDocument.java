...
public class DownloadDocument extends ShortHibernateProcessor {
    ...
    protected void dbProcessing() throws Exception {
        ...
        Document d = TruveoDAOUtil.getFactory().getDocumentDAO().find(documentId);
        ...
        if (!isAdmin()) {
            ...
            for (Iterator it = contests.iterator(); it.hasNext() && !isRegistered;) {
                ...
                if ((ContestStatus.ACTIVE.equals(c.getStatus().getId()) && TruveoDAOUtil.getFactory().getContestRegistrationDAO().find(c, u) != null)
