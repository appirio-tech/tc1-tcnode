...
public class SubmitReview extends Base {
    protected void dbProcessing() throws Exception {
        ...
        ReviewStatus rs = TruveoDAOUtil.getFactory().getReviewStatusDAO().find(status);
        if (rs == null) {
            addError(Constants.REVIEW_STATUS_ID, "Invalid review status specified");
        }
        if (hasErrors()) {
            getRequest().setAttribute("submission", TruveoDAOUtil.getFactory().getSubmissionDAO().find(submissionId));
            SubmissionReview submissionReview = TruveoDAOUtil.getFactory().getSubmissionReviewDAO().find(submissionId);
            getRequest().setAttribute("reviewStatuses", TruveoDAOUtil.getFactory().getReviewStatusDAO().getReviewStatuses());
            ...
            SubmissionReview sr = TruveoDAOUtil.getFactory().getSubmissionReviewDAO().find(submissionId);
            if (sr == null) {
            ...
            TruveoDAOUtil.getFactory().getSubmissionReviewDAO().saveOrUpdate(sr);
...
*/
            ...
            sr = TruveoDAOUtil.getFactory().getSubmissionReviewDAO().find(submissionId);
/*
...
*/
    ...
    private void sendEmail(User submitter, String text, String fileName, ReviewStatus status, User reviewer, Date submitDate) throws Exception {
        ...
        if (ReviewStatus.PASSED.equals(status.getId())) {
            mail.setSubject("Your TopCoder(R) Truveo submission passed review");
        } else if (ReviewStatus.FAILED.equals(status.getId())) {
            mail.setSubject("Your TopCoder(R) Truveo submission failed review");
        ...
        msgText.append("\n");
        msgText.append("Truveo Developer Challenge");
