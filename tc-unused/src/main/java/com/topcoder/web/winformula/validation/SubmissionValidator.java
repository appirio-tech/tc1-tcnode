...
public class SubmissionValidator implements Validator {
    ...
    public static MimeType getMimeType(UploadedFile submission) throws FileDoesNotExistException, PersistenceException {
        MimeType mt = WinformulaDAOUtil.getFactory().getMimeTypeDAO().find(submission.getContentType());
        if (mt == null) {
            ...
            for (WinformulaFileType ft : WinformulaDAOUtil.getFactory().getFileTypeDAO().getFileTypes()) {
