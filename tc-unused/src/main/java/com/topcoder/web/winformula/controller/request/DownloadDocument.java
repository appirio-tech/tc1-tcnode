...
public class DownloadDocument extends ShortHibernateProcessor {
    ...
    protected void dbProcessing() throws Exception {
        ...
        Document d = WinformulaDAOUtil.getFactory().getDocumentDAO().find(documentId);
        ...
        if (!isAdmin()) {
            ...
            for (Iterator it = contests.iterator(); it.hasNext() && !isRegistered;) {
                ...
                if ((ContestStatus.ACTIVE.equals(c.getStatus().getId()) && WinformulaDAOUtil.getFactory().getContestRegistrationDAO().find(c, u) != null)
