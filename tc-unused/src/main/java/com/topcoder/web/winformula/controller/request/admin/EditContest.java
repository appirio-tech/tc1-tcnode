...
import com.topcoder.web.winformula.validation.*;
...
public class EditContest extends Base {
    ...
    protected void dbProcessing() throws Exception {
        ...
        if ("".equals(StringUtils.checkNull(contestStatusId))) {
            ...
            status = WinformulaDAOUtil.getFactory().getContestStatusDAO().find(new Integer(contestStatusId));
            if (status == null) {
                addError(Constants.CONTEST_STATUS_ID, "Please choose a valid contest status.");
            }
        }
        ...
        if (hasErrors()) {
            ...
            if (!"".equals(StringUtils.checkNull(contestId))) {
                setDefault(Constants.CONTEST_STATUS_ID,
                        WinformulaDAOUtil.getFactory().getContestDAO().find(new Long(contestId)).getStatus().getId());
            } else if (status != null) {
            ...
            if (!"".equals(StringUtils.checkNull(contestId))) {
                ...
                contest = WinformulaDAOUtil.getFactory().getContestDAO().find(new Long(contestId));
            } else {
            ...
            FileTypeDAO fDao = WinformulaDAOUtil.getFactory().getFileTypeDAO();
            HashSet<WinformulaFileType> fts = new HashSet<WinformulaFileType>();
            ...
            if (log.isDebugEnabled()) {
                closeConversation();
                beginCommunication();
                Contest myContest = WinformulaDAOUtil.getFactory().getContestDAO().find(contest.getId());
