...
public class SubmitReview extends Base {
    protected void dbProcessing() throws Exception {
        ...
        ReviewStatus rs = OpenAIMDAOUtil.getFactory().getReviewStatusDAO().find(status);
        if (rs == null) {
            addError(Constants.REVIEW_STATUS_ID, "Invalid review status specified");
        }
        if (hasErrors()) {
            getRequest().setAttribute("submission", OpenAIMDAOUtil.getFactory().getSubmissionDAO().find(submissionId));
            SubmissionReview submissionReview = OpenAIMDAOUtil.getFactory().getSubmissionReviewDAO().find(submissionId);
            getRequest().setAttribute("reviewStatuses", OpenAIMDAOUtil.getFactory().getReviewStatusDAO().getReviewStatuses());
            ...
            SubmissionReview sr = OpenAIMDAOUtil.getFactory().getSubmissionReviewDAO().find(submissionId);
            if (sr == null) {
...
*/
            ...
            sr = OpenAIMDAOUtil.getFactory().getSubmissionReviewDAO().find(submissionId);
/*
...
*/
    ...
    private void sendEmail(User submitter, String text, String fileName, ReviewStatus status, User reviewer, Date submitDate) throws Exception {
        ...
        if (ReviewStatus.PASSED.equals(status.getId())) {
            mail.setSubject("Your TopCoder(R) OpenAIM submission passed review");
        } else if (ReviewStatus.FAILED.equals(status.getId())) {
            mail.setSubject("Your TopCoder(R) OpenAIM submission failed review");
        ...
        msgText.append("\n");
        msgText.append("TopCoder OpenAIM");

        mail.setBody(msgText.toString());
        mail.addToAddress(submitter.getPrimaryEmailAddress().getAddress(), TCSEmailMessage.TO);

        mail.setFromAddress("openaimadmin@topcoder.com", "TopCoder OpenAIM Admin");
