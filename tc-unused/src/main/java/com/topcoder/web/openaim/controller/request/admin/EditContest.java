...
public class EditContest extends Base {
    ...
    protected void dbProcessing() throws Exception {
        ...
        if ("".equals(StringUtils.checkNull(contestStatusId))) {
            ...
            status = OpenAIMDAOUtil.getFactory().getContestStatusDAO().find(new Integer(contestStatusId));
            if (status == null) {
                addError(Constants.CONTEST_STATUS_ID, "Please choose a valid contest status.");
            }
        }
        ...
        if (hasErrors()) {
            ...
            if (!"".equals(StringUtils.checkNull(contestId))) {
                setDefault(Constants.CONTEST_STATUS_ID,
                        OpenAIMDAOUtil.getFactory().getContestDAO().find(new Long(contestId)).getStatus().getId());
            } else if (status != null) {
            ...
            if (!"".equals(StringUtils.checkNull(contestId))) {
                ...
                contest = OpenAIMDAOUtil.getFactory().getContestDAO().find(new Long(contestId));
            } else {
            ...
            FileTypeDAO fDao = OpenAIMDAOUtil.getFactory().getFileTypeDAO();
            HashSet<OpenAIMFileType> fts = new HashSet<OpenAIMFileType>();
            ...
            if (log.isDebugEnabled()) {
                closeConversation();
                beginCommunication();
                Contest myContest = OpenAIMDAOUtil.getFactory().getContestDAO().find(contest.getId());
