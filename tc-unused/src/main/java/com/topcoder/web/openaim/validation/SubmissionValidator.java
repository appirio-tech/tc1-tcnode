...
public class SubmissionValidator implements Validator {
    ...
    public static MimeType getMimeType(UploadedFile submission) throws FileDoesNotExistException, PersistenceException {
        MimeType mt = OpenAIMDAOUtil.getFactory().getMimeTypeDAO().find(submission.getContentType());
        if (mt == null) {
            ...
            for (OpenAIMFileType ft : OpenAIMDAOUtil.getFactory().getFileTypeDAO().getFileTypes()) {
